...
from mani_skill.utils.contrib import apply_pose_to_points, o3d_to_trimesh, trimesh_to_o3d, apply_pose_to_points, \
    normalize_and_clip_in_interval, norm, angle_distance
...
class OpenCabinetEnvBase(BaseEnv):
    ...
    def compute_dense_reward(self, action, state=None):
        ...
        if other_info['ee_close_to_handle']:
            print('ee_close_to_handle')
            stage_reward += 0.5
            vel_reward = normalize_and_clip_in_interval(cabinet_vel, -0.1,
                                                        0.5) * vel_coefficient  # Push vel to positive
    ...
    def compute_dense_reward_self_design(self, action, state=None):
        if state is not None:
            self.set_state(state)
        ...
        angle1 = angle_distance(agent_pose, target_pose)
        angle2 = angle_distance(agent_pose, target_pose_2)
        gripper_angle_err = min(angle1, angle2) / 2.0
        ...
        gripper_angle_rew = -gripper_angle_err
        gripper_coefficient = -1

        rew_ee_handle = -dist_ee_to_handle.mean()
        rew_ee_mid_handle = normalize_and_clip_in_interval(dist_ee_mid_to_handle, -0.01, 4E-3) - 1


        dist_reward = 1 - max(0, (self.target_qpos - self.cabinet.get_qpos()[
        ...
        if other_info['ee_close_to_handle']:
            # rew_ee_handle: [-inf, 0]
            # gripper_angle_rew: [-1, 0]
            # dist_reward: [0, 1]
            reward = rew_ee_handle + gripper_angle_rew + dist_reward
        else:
            # --------- stage 1 -----------------
            # rew_ee_handle: [-inf, 0]
            # gripper_coefficient: -1
            # dist_reward: [0, 1]
            reward = rew_ee_handle + gripper_coefficient + dist_reward
        info_dict = {
            'dist_ee_to_handle': dist_ee_to_handle,
            'angle1': angle1,
            'angle2': angle2,
