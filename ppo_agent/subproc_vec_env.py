...
class MySubprocVecEnv(VecEnv):
    ...
    def reset(self):
        ...
        obs = _flatten_obs([remote.recv() for remote in self.remotes])
        obs = self.convert_obs(obs)
        return obs

    def reset_task(self, task_ids=None):
        self._assert_not_closed()
        if task_ids is None:
            task_ids = [None for _ in self.remotes]
        for remote, task_id in zip(self.remotes, task_ids):
            remote.send(('reset_task', task_id))
